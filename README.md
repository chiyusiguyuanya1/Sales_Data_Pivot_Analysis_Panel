...
一个完整可运行的前后端项目。后端使用 Python + SQLite，前端为 Vue 3 + Element Plus，实现订单查询、摘要卡片、动态透视表、新增订单和自动化 E2E 测试代码。
...
- 当前仓库只保留测试代码和配置
- 浏览器运行时不再由我在本机继续下载
- 你可以使用 VS Code 的 Playwright 插件或本机已有浏览器环境自行执行
...
部分页面截图：![alt text](image.png)![alt text](image-1.png)![alt text](image-2.png)